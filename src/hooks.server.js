...
const directives = {
	...
	'report-uri': [
		`https://sentry.io/api/${public_env.PUBLIC_SENTRY_PROJECT_ID}/security/?sentry_key=${public_env.PUBLIC_SENTRY_KEY}`,
	],
};
...
export async function handle({ event, resolve }) {
	...
	response.headers.set(
		'Expect-CT',
		`max-age=86400, report-uri="https://sentry.io/api/${public_env.PUBLIC_SENTRY_PROJECT_ID}/security/?sentry_key=${public_env.PUBLIC_SENTRY_KEY}"`,
	);
	response.headers.set(
		'Report-To',
		`{group: "csp-endpoint", "max_age": 10886400, "endpoints": [{"url": "https://sentry.io/api/${public_env.PUBLIC_SENTRY_PROJECT_ID}/security/?sentry_key=${public_env.PUBLIC_SENTRY_KEY}"}]}`,
	);
	return response;
}
